...
document.getElementById("depositForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    showMessage("success", formatCurrency(amount) + " deposited successfully!");
    amountInput.value      = "";
    descriptionInput.value = "";

  } catch (err) {
    console.error("Deposit error:", err.code, err.message);
    showMessage("error", "Deposit failed: " + err.message);

  } finally {
    btn.disabled  = false;
    btn.innerHTML = '<i class="fa-solid fa-arrow-down-to-line"></i> Deposit Funds';
  }
});
